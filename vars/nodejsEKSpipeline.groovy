                ...
                steps {
                    withCredentials([string(credentialsId: 'github-token', variable: 'GH_TOKEN')]) {
                        sh """
                            ...
                            REPO="pandalapadu/${configMap.component}"
            ...
            stage('Trivy Scan') {
                steps {
                    script {
                        ...
                        def imageScan = sh(
                            script: "trivy image --scanners vuln --vuln-type os --exit-code 1 --severity HIGH,CRITICAL --ignore-unfixed --format table ${env.APP_NAME}:${env.APP_VERSION}",
                            returnStatus: true
                        )
